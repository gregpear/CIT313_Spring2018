	...
	if (!empty($_POST['productname' . $x]))  {
		...
		switch ($thisProduceType) {
			...
			case "v": //veg
				...
				$produceItem = new veggie($x, $thisProductName, $thisProductPriceType, $thisProductPrice); // $_thisProductName variable is not correct, replaced it with $thisProductName
				$typeType = "Veggie";
				break;
		}
		
		if($produceItem->price_type == 2) {
			$pType = "Each"; // Semicolon is missing, Added semicolon
		}
		else {
			$pType = "Per Lb";
		}
